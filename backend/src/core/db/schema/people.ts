...
export const pidPerson = pgTable(
  ...
  (table) => ({
    idxPerLastFirst: index("idx_per_last_first").on(
      table.perLastName,
      table.perFirstName,
    ),
...
export const pidPersonContact = pgTable(
  "pid_person_contact",
  {
    ...
    pcoContactTypeCode: varchar("pco_contact_type_code", {
      length: 50,
    }).notNull(), // EMAIL, MOBILE, PHONE, ADDRESS, OTHER
    ...
    // Audit columns
    pcoCreatedAt: timestamp("pco_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidPersonIdentifier = pgTable(
  "pid_person_identifier",
  {
    ...
    idnIdentifierTypeCode: varchar("idn_identifier_type_code", {
      length: 50,
    }).notNull(), // NATIONAL_ID, PASSPORT, TAX_PIN, etc.
    idnIdentifierValue: varchar("idn_identifier_value", {
      length: 100,
    }).notNull(),
    ...
    // Audit columns
    idnCreatedAt: timestamp("idn_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidEmployee = pgTable(
  "pid_employee",
  {
    ...
    // Audit columns
    empCreatedAt: timestamp("emp_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  ...
  (table) => ({
    ...
    uqEmpTenantNumber: unique("uq_emp_tenant_number").on(
      table.empTenantId,
      table.empEmployeeNumber,
    ),
    uqEmpTenantPerson: unique("uq_emp_tenant_person").on(
      table.empTenantId,
      table.empPerId,
    ),
...
export const pidEmpContract = pgTable(
  "pid_emp_contract",
  {
    ...
    ctrContractTypeCode: varchar("ctr_contract_type_code", {
      length: 50,
    }).notNull(), // PERMANENT, FIXED_TERM, INTERNSHIP etc.
    ctrStartDate: date("ctr_start_date").notNull(),
    ctrEndDate: date("ctr_end_date"),
    ctrProbationEndDate: date("ctr_probation_end_date"),
    ctrStandardHoursPerWeek: decimal("ctr_standard_hours_per_week", {
      precision: 5,
      scale: 2,
    }),
    ctrStandardDaysPerWeek: decimal("ctr_standard_days_per_week", {
      precision: 3,
      scale: 2,
    }),
    ...
    // Audit columns
    ctrCreatedAt: timestamp("ctr_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidEmpStatusHistory = pgTable(
  "pid_emp_status_history",
  {
    ...
    // Audit columns
    eshCreatedAt: timestamp("esh_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidDependent = pgTable(
  "pid_dependent",
  {
    ...
    depRelationshipCode: varchar("dep_relationship_code", {
      length: 50,
    }).notNull(), // SPOUSE, CHILD, PARENT, OTHER
    depDateOfBirth: date("dep_date_of_birth"),
    depIncludedInHealthCover: boolean("dep_included_in_health_cover")
      .notNull()
      .default(false),
    depWellnessEligible: boolean("dep_wellness_eligible")
      .notNull()
      .default(false),

    // Audit columns
    depCreatedAt: timestamp("dep_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  ...
  (table) => ({
    ...
    idxDepTenantEmp: index("idx_dep_tenant_emp").on(
      table.depTenantId,
      table.depEmpId,
    ),
  }),
...
export const pidQualification = pgTable(
  "pid_qualification",
  {
    ...
    qlfQualificationTypeCode: varchar("qlf_qualification_type_code", {
      length: 50,
    }).notNull(), // EDUCATION, PROFESSIONAL etc.
    qlfInstitution: varchar("qlf_institution", { length: 255 }),
    qlfQualificationName: varchar("qlf_qualification_name", {
      length: 255,
    }).notNull(),
    ...
    // Audit columns
    qlfCreatedAt: timestamp("qlf_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidEmploymentHistory = pgTable(
  "pid_employment_history",
  {
    ...
    // Audit columns
    pehCreatedAt: timestamp("peh_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidWellnessProfile = pgTable(
  "pid_wellness_profile",
  {
    ...
    wepPreferredChannelCode: varchar("wep_preferred_channel_code", {
      length: 50,
    }), // EMAIL, SMS, WHATSAPP, APP etc.
    wepLastZhepSyncAt: timestamp("wep_last_zhep_sync_at", {
      withTimezone: true,
    }),

    // Audit columns
    wepCreatedAt: timestamp("wep_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const pidWellnessProfileTag = pgTable(
  "pid_wellness_profile_tag",
  {
    ...
    wptFirstSeenAt: timestamp("wpt_first_seen_at", {
      withTimezone: true,
    }).notNull(),
    wptLastUpdatedAt: timestamp("wpt_last_updated_at", {
      withTimezone: true,
    }).notNull(),
    wptIsActive: boolean("wpt_is_active").notNull().default(true),

    // Audit columns
    wptCreatedAt: timestamp("wpt_created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  ...
  (table) => ({
    ...
    uqWptProfileTag: unique("uq_wpt_profile_tag").on(
      table.wptTenantId,
      table.wptWepId,
      table.wptTagCode,
    ),
