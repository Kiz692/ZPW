...
describe('PersonService', () => {
  ...
  describe('getById', () => {
    it('should get person by ID', async () => {
      const person = await personService.create(TestFactories.createPerson(), 1);
  ...
  describe('searchByName', () => {
    it('should search persons by name', async () => {
      await personService.create(TestFactories.createPerson({ perFirstName: 'John', perLastName: 'Doe' }), 1);
    ...
    it('should throw error if search term too short', async () => {
      await expect(personService.searchByName('J')).rejects.toThrow('at least 2 characters');
    });
  });
  ...
  describe('update', () => {
    it('should update person', async () => {
      const person = await personService.create(TestFactories.createPerson(), 1);
      const updated = await personService.update(person.perId, { perFirstName: 'Updated' }, 1);
