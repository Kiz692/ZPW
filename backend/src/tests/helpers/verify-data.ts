 ...
 */
export async function getStatusHistoryCount(employeeId: number, tenantId: number): Promise<number> {
  const result = await db
    .select({ count: sql<number>`count(*)` })
 ...
 */
export async function verifyEmployeeExists(employeeId: number, tenantId: number): Promise<boolean> {
  const result = await db
    .select()
    .from(pidEmployee)
    .where(
 ...
 */
export async function verifyContractExists(contractId: number, tenantId: number): Promise<boolean> {
  const result = await db
    .select()
    .from(pidEmpContract)
    .where(
 ...
 */
export async function getStatusHistoryFromDb(employeeId: number, tenantId: number) {
  return await db
    .select()
    .from(pidEmpStatusHistory)
    .where(
