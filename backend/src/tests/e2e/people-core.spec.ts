...
test.describe('People Core E2E Tests', () => {
  ...
  test('should display employee count correctly', async ({ page, request }) => {
    ...
    const apiResponse = await request.get(`${API_BASE_URL}/api/v1/people/employees`, {
      headers: {
    ...
    if (expectedCount > 0) {
      ...
      const employeeElements = await page.locator('[data-testid="employee-row"], tr, .employee-item').count();
  ...
  test('should create person and employee flow', async ({ page, request }) => {
    ...
    const personResponse = await request.post(`${API_BASE_URL}/api/v1/people/persons`, {
      headers: {
        'X-Tenant-ID': String(DEMO_TENANT_ID),
        'X-User-ID': '1',
    ...
    const employeeResponse = await request.post(`${API_BASE_URL}/api/v1/people/employees`, {
      headers: {
        'X-Tenant-ID': String(DEMO_TENANT_ID),
        'X-User-ID': '1',
        'Content-Type': 'application/json',
