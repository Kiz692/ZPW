...
export type WellnessProfileTagInsert = typeof pidWellnessProfileTag.$inferInsert;
export type WellnessProfileTagUpdate = Partial<WellnessProfileTagInsert>;

export class WellnessProfileTagRepository {
  async findById(id: number, tenantId: number): Promise<WellnessProfileTag | null> {
  ...
  async findByWellnessProfileId(wellnessProfileId: number, tenantId: number): Promise<WellnessProfileTag[]> {
    return await db
      .select()
      .from(pidWellnessProfileTag)
      .where(
  ...
  async findByTagCode(wellnessProfileId: number, tagCode: string, tenantId: number): Promise<WellnessProfileTag | null> {
    const [result] = await db
      .select()
      .from(pidWellnessProfileTag)
      .where(
  ...
  async create(data: WellnessProfileTagInsert, userId?: number): Promise<WellnessProfileTag> {
    const now = new Date();
    ...
    const [result] = await db.insert(pidWellnessProfileTag).values(insertData).returning();
    return result;
  }

  async update(id: number, data: WellnessProfileTagUpdate, tenantId: number, userId?: number): Promise<WellnessProfileTag | null> {
    const now = new Date();
