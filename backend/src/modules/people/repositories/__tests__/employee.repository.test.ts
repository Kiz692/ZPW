...
describe("EmployeeRepository", () => {
  ...
  describe("tenant isolation", () => {
    it("should only find employees for specified tenant", async () => {
      const person1 = await personRepo.create(
        TestFactories.createPerson({ perFirstName: "John" }),
      );
      const person2 = await personRepo.create(
        TestFactories.createPerson({ perFirstName: "Jane" }),
      );

      const emp1 = await employeeRepo.create(
        TestFactories.createEmployee(1, person1.perId, {
          empEmployeeNumber: "EMP001",
        }),
      );
      const emp2 = await employeeRepo.create(
        TestFactories.createEmployee(2, person2.perId, {
          empEmployeeNumber: "EMP001",
        }),
      );
    ...
    it("should not find employee from different tenant", async () => {
      ...
      const employee = await employeeRepo.create(
        TestFactories.createEmployee(1, person.perId),
      );
  ...
  describe("updateStatus", () => {
    it("should update employee status", async () => {
      ...
      const updatedDate =
        updated?.empCurrentStatusEffectiveDate instanceof Date
      ...
      expect(updatedDate.toISOString().split("T")[0]).toBe(
