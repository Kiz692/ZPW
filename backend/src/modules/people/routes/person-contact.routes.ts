...
import {
  personContactCreateSchema,
  personContactUpdateSchema,
} from "../schemas/person-contact.schemas.js";
...
export async function registerPersonContactRoutes(app: FastifyInstance) {
  ...
  app.post(
      ...
      schema: {
        ...
        body: {
          ...
          properties: {
            pcoContactTypeCode: {
              type: "string",
              enum: ["EMAIL", "MOBILE", "PHONE", "ADDRESS", "OTHER"],
            },
        ...
        preValidation: async (request: any, reply: any) => {
          try {
            personContactCreateSchema.parse({
              ...request.body,
              pcoPerId: Number(request.params.personId),
            });
          } catch (error: any) {
    ...
    async (request: any, reply) => {
      try {
        ...
        const validatedBody = personContactCreateSchema.parse({
          ...request.body,
          pcoPerId: Number(request.params.personId),
        });
  ...
  app.get(
    ...
    async (request: any, reply) => {
      const contacts = await contactService.getByPersonId(
