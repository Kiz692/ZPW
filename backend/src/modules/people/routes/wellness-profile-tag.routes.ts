...
import { wellnessProfileTagCreateSchema, wellnessProfileTagUpdateSchema } from '../schemas/wellness-profile-tag.schemas.js';
...
export async function registerWellnessProfileTagRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request: any, reply) => {
      const tenantId = requireTenant(request);
      const tags = await tagService.getByWellnessProfileId(Number(request.params.wellnessProfileId), tenantId);
  ...
  app.get(
    ...
    async (request: any, reply) => {
      try {
        const tenantId = requireTenant(request);
        const tag = await tagService.getById(Number(request.params.id), tenantId);
