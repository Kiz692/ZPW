...
import { contractCreateSchema, contractUpdateSchema } from '../schemas/contract.schemas.js';
...
export async function registerContractRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        const contract = await contractService.create(request.body as any, userId);
        return reply.status(201).send(contract);
      } catch (error: any) {
        if (error.message.includes('overlap') || error.message.includes('date')) {
  ...
  app.get(
    ...
    async (request: any, reply) => {
      try {
        ...
        const contract = await contractService.getById(Number(request.params.id), tenantId);
  ...
  app.get(
    ...
    async (request: any, reply) => {
      const tenantId = requireTenant(request);
      const contracts = await contractService.getByEmployeeId(Number(request.params.employeeId), tenantId);
  ...
  app.put(
    ...
    async (request: any, reply) => {
      try {
        const tenantId = requireTenant(request);
        const userId = requireUser(request);
        const contract = await contractService.update(Number(request.params.id), request.body as any, tenantId, userId);
        return reply.send(contract);
      } catch (error: any) {
  ...
  app.delete(
    ...
    async (request: any, reply) => {
      try {
        const tenantId = requireTenant(request);
        const userId = requireUser(request);
        await contractService.delete(Number(request.params.id), tenantId, userId);
