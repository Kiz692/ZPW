...
} from "../schemas/dependent.schemas.js";
...
export async function registerDependentRoutes(app: FastifyInstance) {
  ...
  app.post(
    "/employees/:employeeId/dependents",
    {
      schema: {
        ...
        preValidation: async (request: any, reply: any) => {
          try {
            dependentCreateSchema.parse({
              ...request.body,
              depTenantId: 1,
              depEmpId: Number(request.params.employeeId),
            });
          } catch (error: any) {
    ...
    async (request: any, reply) => {
      try {
        ...
        const validatedBody = dependentCreateSchema.parse({
          ...request.body,
  ...
  app.get(
    ...
    async (request: any, reply) => {
      const tenantId = requireTenant(request);
      const dependents = await dependentService.getByEmployeeId(
  ...
  app.get(
    ...
    async (request: any, reply) => {
      try {
        const tenantId = requireTenant(request);
        const dependent = await dependentService.getById(
  ...
  app.put(
    ...
    async (request: any, reply) => {
      try {
        ...
        const dependent = await dependentService.update(
          Number(request.params.id),
          validatedBody,
          tenantId,
          userId,
