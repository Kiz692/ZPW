...
describe('PersonService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw error if required fields missing', async () => {
      await expect(personService.create({ perFirstName: 'John' } as any, 1)).rejects.toThrow('required');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw error if person not found', async () => {
      await expect(personService.update(99999, { perFirstName: 'Jane' }, 1)).rejects.toThrow('not found');
    });
  });
