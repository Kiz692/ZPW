...
export class PersonContactService extends BaseService {
  ...
  async create(
    ...
    this.validateRequired(data, ['pcoPerId', 'pcoContactTypeCode', 'pcoContactValue']);
    ...
    if (data.pcoIsPrimary) {
      const existingPrimary = await this.contactRepo.findPrimaryContact(data.pcoPerId, data.pcoContactTypeCode);
      if (existingPrimary) {
        await this.contactRepo.update(existingPrimary.pcoId, { pcoIsPrimary: false }, userId);
      }
    }
  ...
  async update(
    ...
    if (data.pcoContactValue) {
      const contactType = data.pcoContactTypeCode || existing.pcoContactTypeCode;
